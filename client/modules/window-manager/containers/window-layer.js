    ...
    drop: (props, monitor) => {
        ...
        switch(item.dragType) {
            ...
            case dragTypes.windowResizerType:
                delta = monitor.getDifferenceFromInitialOffset();
                if (item.which.indexOf('t') !== -1) y = item.position.top + delta.y;
                if (item.which.indexOf('l') !== -1) x = item.position.left + delta.x;
                if (item.which.indexOf('b') !== -1) y = item.position.top + item.position.height + delta.y;
                if (item.which.indexOf('r') !== -1) x = item.position.left + item.position.width + delta.x;
...
const composer = (context, onData) => {
    ...
    LocalState.setDefault(windowResizing(context._id, false));

    const resizing = LocalState.get(windowResizing(context._id));
    const layerHidden = LocalState.get(layerHiddenStateVar(context._id));
    onData(null, { hiddenLayer: !resizing && layerHidden });
}
